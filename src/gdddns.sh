...
gd_ip_addr=$(echo $dns_data | cut -d ',' -f 1 | tr -d '"' | cut -d ":" -f 2)
...
if [ "$gd_ip_addr" != "$ip_addr" -a $ip_addr != "" ]; then
  ...
  curl -s -X PUT "https://api.godaddy.com/v1/domains/$DOMAIN/records/A/$HOSTNAME" -H "$GD_AUTH_STR" -H "Content-Type: application/json" -d "[{\"data\": \"${ip_addr}\"}]"
fi
